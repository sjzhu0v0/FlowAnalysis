...
void c2_m2(
    ...
    string path_file_output =
        "/data/work/work_alice/Analysis/JpsiFlow/c2_m2/output/JpsiFlow.root") {
#ifdef roothttp
  ...
  cout << endl;
  for (int i = 0; i < vec_f1_resBkg.size(); i++) {
    cout << "index:\t" << i << "\t\t";
    vec_f1_resBkg[i]->Print();
  }

  cout << endl;
  for (int i = 0; i < vec_h1_mass_fit.size(); i++) {
    cout << "index:\t" << i << "\t\t";
    vec_h1_mass_fit[i]->Print();
  }

  cout << endl;
  for (int i = 0; i < vec_h1_mass_uls.size(); i++) {
    cout << "index:\t" << i << "\t\t";
    vec_h1_mass_uls[i]->Print();
  }
  ...
  TProfile3D *fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im =
      (TProfile3D *)vec_h1_target[12];
  TProfile3D *fProfileTarget_EtaWidth_Pt_Mass_c2_m2_re =
      (TProfile3D *)vec_h1_target[13];
  TProfile3D *fProfileTargetbg_EtaWidth_Pt_Mass_c2_m2_im =
      (TProfile3D *)vec_h1_targetbg[12];
  TProfile3D *fProfileTargetbg_EtaWidth_Pt_Mass_c2_m2_re =
      (TProfile3D *)vec_h1_targetbg[13];

  double *bins_widthEta =
      fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im->GetXaxis()->GetXbins()->fArray;
  double *bins_pt =
      fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im->GetYaxis()->GetXbins()->fArray;
  double *bins_mass =
      fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im->GetZaxis()->GetXbins()->fArray;
  int n_bins_eta_width =
      fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im->GetXaxis()->GetNbins();
  int n_bins_pt =
      fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im->GetYaxis()->GetNbins();
  int n_bins_mass =
      fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im->GetZaxis()->GetNbins();
  ...
  TH3D *h3target_widthEta_pt_mass_C2_2 =
      new TH3D("h3target_widthEta_pt_mass_C2_2",
               "h3target_widthEta_pt_mass_C2_2;#eta width;p_{T} "
               "[GeV/c];Mass_{e^{+}e^{-}}[GeV/c]",
               n_bins_eta_width, bins_widthEta, n_bins_pt, bins_pt, n_bins_mass,
               bins_mass); // c2_m2
  TH3D *h3targetbg_widthEta_pt_mass_C2_2 =
      new TH3D("h3targetbg_widthEta_pt_mass_C2_2",
               "h3targetbg_widthEta_pt_mass_C2_2;#eta width;p_{T} "
               "[GeV/c];Mass_{e^{+}e^{-}}[GeV/c]",
               n_bins_eta_width, bins_widthEta, n_bins_pt, bins_pt, n_bins_mass,
               bins_mass);
  TH3D *h3target_widthEta_pt_mass_v2_2 =
      new TH3D("h3target_widthEta_pt_mass_v2_2",
               "h3target_widthEta_pt_mass_v2_2;#eta width;p_{T} "
               "[GeV/c];Mass_{e^{+}e^{-}}[GeV/c]",
               n_bins_eta_width, bins_widthEta, n_bins_pt, bins_pt, n_bins_mass,
               bins_mass); // c2_m2
  TH3D *h3targetbg_widthEta_pt_mass_v2_2 =
      new TH3D("h3targetbg_widthEta_pt_mass_v2_2",
               "h3targetbg_widthEta_pt_mass_v2_2;#eta width;p_{T} "
               "[GeV/c];Mass_{e^{+}e^{-}}[GeV/c]",
               n_bins_eta_width, bins_widthEta, n_bins_pt, bins_pt, n_bins_mass,
               bins_mass);
  ...
  for (int i_bin_widthEta = 1; i_bin_widthEta <= n_bins_eta_width;
    ...
    for (int i_bin_pt = 1; i_bin_pt <= n_bins_pt; i_bin_pt++) {
      for (int i_bin_mass = 1; i_bin_mass <= n_bins_mass; i_bin_mass++) {
        double c2_m2_im_target =
            fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im->GetBinContent(
                i_bin_widthEta, i_bin_pt, i_bin_mass);
        double c2_m2_re_target =
            fProfileTarget_EtaWidth_Pt_Mass_c2_m2_re->GetBinContent(
                i_bin_widthEta, i_bin_pt, i_bin_mass);

        double c2_m2_im_target_err =
            fProfileTarget_EtaWidth_Pt_Mass_c2_m2_im->GetBinError(
                i_bin_widthEta, i_bin_pt, i_bin_mass);
        double c2_m2_re_target_err =
            fProfileTarget_EtaWidth_Pt_Mass_c2_m2_re->GetBinError(
                i_bin_widthEta, i_bin_pt, i_bin_mass);
        ...
        double N_target_err = h3_widthEta_pt_mass_target->GetBinError(
            i_bin_widthEta, i_bin_pt, i_bin_mass);
        MComplex c2_m2_target(c2_m2_re_target, c2_m2_im_target,
                              c2_m2_re_target_err, c2_m2_im_target_err);
        h3target_widthEta_pt_mass_C2_2->SetBinContent(
            i_bin_widthEta, i_bin_pt, i_bin_mass, c2_m2_target.Mag());
        h3target_widthEta_pt_mass_C2_2->SetBinError(
            i_bin_widthEta, i_bin_pt, i_bin_mass, c2_m2_target.MagErr());

        double c2_m2_im_targetbg =
            fProfileTargetbg_EtaWidth_Pt_Mass_c2_m2_im->GetBinContent(
                i_bin_widthEta, i_bin_pt, i_bin_mass);
        double c2_m2_re_targetbg =
            fProfileTargetbg_EtaWidth_Pt_Mass_c2_m2_re->GetBinContent(
                i_bin_widthEta, i_bin_pt, i_bin_mass);

        double c2_m2_im_targetbg_err =
            fProfileTargetbg_EtaWidth_Pt_Mass_c2_m2_im->GetBinError(
                i_bin_widthEta, i_bin_pt, i_bin_mass);
        double c2_m2_re_targetbg_err =
            fProfileTargetbg_EtaWidth_Pt_Mass_c2_m2_re->GetBinError(
                i_bin_widthEta, i_bin_pt, i_bin_mass);
        ...
        double N_targetbg_err = h3_widthEta_pt_mass_targetbg->GetBinError(
            i_bin_widthEta, i_bin_pt, i_bin_mass);
        MComplex c2_m2_targetbg(c2_m2_re_targetbg, c2_m2_im_targetbg,
                                c2_m2_re_targetbg_err, c2_m2_im_targetbg_err);
        h3targetbg_widthEta_pt_mass_C2_2->SetBinContent(
            i_bin_widthEta, i_bin_pt, i_bin_mass, c2_m2_targetbg.Mag());
        h3targetbg_widthEta_pt_mass_C2_2->SetBinError(
            i_bin_widthEta, i_bin_pt, i_bin_mass, c2_m2_targetbg.MagErr());

        double etaWidth = bins_widthEta[i_bin_widthEta - 1];
        int index_deltaEta_reference =
            h1_v2_2_reference->GetXaxis()->FindBin(etaWidth);
        double v2_reference =
            h1_v2_2_reference->GetBinContent(index_deltaEta_reference);
        double v2_reference_err =
            h1_v2_2_reference->GetBinError(index_deltaEta_reference);

        double v2_target = c2_m2_target.Mag() / v2_reference;
        double v2_target_err = sqrt(
            pow(c2_m2_target.MagErr() / v2_reference, 2) +
            pow(c2_m2_target.Mag() * v2_reference_err / pow(v2_reference, 2),
                2));
        double v2_targetbg = c2_m2_targetbg.Mag() / v2_reference;
        double v2_targetbg_err = sqrt(
            pow(c2_m2_targetbg.MagErr() / v2_reference, 2) +
            pow(c2_m2_targetbg.Mag() * v2_reference_err / pow(v2_reference, 2),
                2));

        h3target_widthEta_pt_mass_v2_2->SetBinContent(i_bin_widthEta, i_bin_pt,
                                                      i_bin_mass, v2_target);
        h3target_widthEta_pt_mass_v2_2->SetBinError(i_bin_widthEta, i_bin_pt,
                                                    i_bin_mass, v2_target_err);
        h3targetbg_widthEta_pt_mass_v2_2->SetBinContent(
            i_bin_widthEta, i_bin_pt, i_bin_mass, v2_targetbg);
        h3targetbg_widthEta_pt_mass_v2_2->SetBinError(
            i_bin_widthEta, i_bin_pt, i_bin_mass, v2_targetbg_err);
      }
    }
  }
  ...
  int index_binWidth_requested =
      h3target_widthEta_pt_mass_v2_2->GetXaxis()->FindBin(binWidth_requested);

  TH2F *h2_v2_2_sig_mass_pt = new TH2F(
      "h2_v2_2_target", ";Mass_{e^{+}e^{-}}[GeV/c^{2}];#it{p}_{T} [GeV/c]",
      n_bins_mass, bins_mass, n_bins_pt, bins_pt);

  vector<TH1D *> vec_h1_mass_fit_flow;
  for (int i_bin_pt = 1; i_bin_pt <= n_bins_pt; i_bin_pt++) {
    TH1D *h1_targetbg_mass =
        (TH1D *)h3targetbg_widthEta_pt_mass_v2_2->ProjectionZ(
    ...
    for (int i_bin_mass = 1; i_bin_mass <= n_bins_mass; i_bin_mass++) {
      double value_binCenter_mass =
          h3target_widthEta_pt_mass_v2_2->GetZaxis()->GetBinCenter(i_bin_mass);
      ...
      double v2_target = h3target_widthEta_pt_mass_v2_2->GetBinContent(
          index_binWidth_requested, i_bin_pt, i_bin_mass);
      double v2_target_err = h3target_widthEta_pt_mass_v2_2->GetBinError(
          index_binWidth_requested, i_bin_pt, i_bin_mass);
      double v2_targetbg = f1_Bkg_flow.Eval(value_binCenter_mass);
      // double v2_targetbg = h3targetbg_widthEta_pt_mass_v2_2->GetBinContent(
      //     index_binWidth_requested, i_bin_pt, i_bin_mass);
      // double v2_targetbg_err = h3targetbg_widthEta_pt_mass_v2_2->GetBinError(
      //     index_binWidth_requested, i_bin_pt, i_bin_mass);
      ...
      h2_v2_2_sig_mass_pt->SetBinContent(i_bin_mass, i_bin_pt, v2_sig);
      h2_v2_2_sig_mass_pt->SetBinError(i_bin_mass, i_bin_pt, v2_sig_err);
    }
  }

  int index_bin_mass_flow_low = h2_v2_2_sig_mass_pt->GetXaxis()->FindBin(2.72);
  int index_bin_mass_flow_high = h2_v2_2_sig_mass_pt->GetXaxis()->FindBin(3.2);

  TH1F *h1_v2_2_sig_pt =
      new TH1F("h1_v2_2_sig_pt", ";#it{p}_{T} [GeV/c];v_{2}{2}_{3 sub events}",
               n_bins_pt, bins_pt);
  ...
  for (int i_bin_pt = 1; i_bin_pt <= n_bins_pt; i_bin_pt++) {
    ...
    for (int i_bin_mass = index_bin_mass_flow_low;
      ...
      double bin_err = h2_v2_2_sig_mass_pt->GetBinError(i_bin_mass, i_bin_pt);
      double bin_content =
          h2_v2_2_sig_mass_pt->GetBinContent(i_bin_mass, i_bin_pt);
    ...
    cout << "pt:\t" << bins_pt[i_bin_pt - 1] << "\t" << value << "\t"
         << value_err << endl;
    h1_v2_2_sig_pt->SetBinContent(i_bin_pt, value);
    h1_v2_2_sig_pt->SetBinError(i_bin_pt, value_err);
  }

  TCanvas *c1_C2_4 = new TCanvas("c1_C2_4", "c1_C2_4", 600, 600);
  c1_C2_4->Divide(2, 2);
  c1_C2_4->cd(1);
  h3target_widthEta_pt_mass_C2_2
      ->ProjectionZ("h3target_widthEta_pt_mass_C2_4_z", 4, 4, 1, 1)
      ->Draw();
  c1_C2_4->cd(2);
  h3targetbg_widthEta_pt_mass_C2_2
      ->ProjectionZ("h3targetbg_widthEta_pt_mass_C2_4_z", 4, 4, 1, 1)
      ->Draw();
  c1_C2_4->cd(3);
  h3target_widthEta_pt_mass_v2_2
      ->ProjectionZ("h3target_widthEta_pt_mass_v2_2_z", 5, 5, 2, 2)
      ->Draw();
  c1_C2_4->cd(4);
  TH1D *h1_h3targetbg_widthEta_pt_mass_v2_2_z =
      (TH1D *)h3targetbg_widthEta_pt_mass_v2_2->ProjectionZ(
          "h3targetbg_widthEta_pt_mass_v2_2_z", 5, 5, 2, 2);
  TF1 *f1 = new TF1("f1", "pol1", 2.4, 3.6);
  h1_h3targetbg_widthEta_pt_mass_v2_2_z->Fit(f1, "R");
  h1_h3targetbg_widthEta_pt_mass_v2_2_z->Draw();

  TCanvas *c1_C2_4_QA = new TCanvas("c1_C2_2_QA", "c1_C2_4_QA", 600, 600);
  c1_C2_4_QA->Divide(2, 2);
  c1_C2_4_QA->cd(1);
  h2_v2_2_sig_mass_pt->Draw("colz");
  c1_C2_4_QA->cd(2);
  h1_v2_2_sig_pt->Draw();
  c1_C2_4_QA->cd(3);
  TH1D *h1_h2_v2_2_sig_mass_x =
      (TH1D *)h2_v2_2_sig_mass_pt->ProjectionX("h2_v2_2_sig_mass_x", 2, 2);
  h1_h2_v2_2_sig_mass_x->GetYaxis()->SetRangeUser(-1, 1.);
  h1_h2_v2_2_sig_mass_x->GetYaxis()->SetTitle("v_{2}{2}_{Signal}");
  TF1 *f1_h1_h2_v2_2_sig_mass_x =
      new TF1("f1_h1_h2_v2_2_sig_mass_x", "pol0", 2.72, 3.2);
  h1_h2_v2_2_sig_mass_x->Fit(f1_h1_h2_v2_2_sig_mass_x, "R");
  h1_h2_v2_2_sig_mass_x->Draw();
  c1_C2_4_QA->cd(4);
